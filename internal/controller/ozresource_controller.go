...
const (
	ResourceFinalizer = "gateway.openzro.io/resource"
)
...
// OZResourceReconciler reconciles a OZResource object
type OZResourceReconciler struct {
	client.Client
...
func (r *OZResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if result != nil {
		nbResource.Status.Conditions = openzrov1.OZConditionFalse("internalError", fmt.Sprintf("Error occurred handling groups: %v", err))
		return *result, err
	}

	resource, err := r.handleopenZroResource(ctx, nbResource, groupIDs, logger)
	if err != nil && nerrors.Is(err, errDuplicateResource) {
		return ctrl.Result{RequeueAfter: defaultRequeueAfter}, nil
	}

	if err != nil {
		nbResource.Status.Conditions = openzrov1.OZConditionFalse("internalError", fmt.Sprintf("Error occurred handling openZro Network Resource: %v", err))
		return ctrl.Result{}, err
	}
	...
	if err != nil {
		nbResource.Status.Conditions = openzrov1.OZConditionFalse("internalError", fmt.Sprintf("Error occurred handling policy changes: %v", err))
		return ctrl.Result{}, err
	}

	nbResource.Status.Conditions = openzrov1.OZConditionTrue()

	return ctrl.Result{}, nil
}

func (r *OZResourceReconciler) handlePolicyCreate(ctx context.Context, nbResource *openzrov1.OZResource, req ctrl.Request, policy string, nbPolicy *openzrov1.OZPolicy, logger logr.Logger) error {
	...
	*nbPolicy = openzrov1.OZPolicy{
	...
	if kerrors.IsAlreadyExists(err) {
		...
		nbPolicy.Annotations["openzro.io/generated-by"] = req.NamespacedName.String()
		nbPolicy.Spec = openzrov1.OZPolicySpec{
			Name:          name,
			Description:   "Generated by " + req.NamespacedName.String(),
			SourceGroups:  nbResource.Spec.PolicySourceGroups,
			Bidirectional: true,
...
func (r *OZResourceReconciler) handlePolicyAddUpdate(ctx context.Context, req ctrl.Request, nbResource *openzrov1.OZResource, policy string, groupIDs []string, logger logr.Logger) error {
	...
	if kerrors.IsNotFound(err) && r.AllowAutomaticPolicyCreation {
		...
		logger.Error(errKubernetesAPI, "error getting OZPolicy", "err", err, "policyName", policy)
		return err
	}
	...
	if _, ok := nbResource.Status.PolicyNameMapping[policy]; ok {
		...
		if v, ok := nbPolicy.Annotations["openzro.io/generated-by"]; !ok || v != req.NamespacedName.String() {
			if nbPolicy.Annotations == nil {
				nbPolicy.Annotations = make(map[string]string)
			}
			nbPolicy.Annotations["openzro.io/generated-by"] = req.NamespacedName.String()
	...
	if updatePolicyStatus {
		err := r.Client.Status().Update(ctx, &nbPolicy)
		if err != nil {
			logger.Error(errKubernetesAPI, "error updating OZPolicy", "err", err, "policyName", policy)
			return err
		}
	}
...
func (r *OZResourceReconciler) handlePolicyDelete(ctx context.Context, req ctrl.Request, nbResource *openzrov1.OZResource, specPolicies []string, policy string, logger logr.Logger) error {
	var nbPolicy openzrov1.OZPolicy
	if !slices.Contains(specPolicies, policy) {
		...
		if !kerrors.IsNotFound(err) {
			...
			if _, ok := nbResource.Status.PolicyNameMapping[policy]; ok {
				...
				if err != nil {
					logger.Error(errKubernetesAPI, "error deleting OZPolicy", "err", err, "policyName", policy)
					return err
				}
...
// handlePolicy update OZPolicy if defined to add self reference to policy status
func (r *OZResourceReconciler) handlePolicy(ctx context.Context, req ctrl.Request, nbResource *openzrov1.OZResource, groupIDs []string, logger logr.Logger) error {
...
func (r *OZResourceReconciler) handleGroupUpdate(ctx context.Context, nbResource *openzrov1.OZResource, groupIDs []string, resource *api.NetworkResource, logger logr.Logger) error {
	...
	if diffFound {
		_, err := r.OpenZro.Networks.Resources(nbResource.Spec.NetworkID).Update(ctx, resource.Id, api.NetworkResourceRequest{
		...
		if err != nil {
			logger.Error(erropenZroAPI, "error updating resource", "err", err)
			return err
		}
	}
...
// handleopenZroResource sync openZro Network Resource
func (r *OZResourceReconciler) handleopenZroResource(ctx context.Context, nbResource *openzrov1.OZResource, groupIDs []string, logger logr.Logger) (*api.NetworkResource, error) {
	var resource *api.NetworkResource
	var err error
	if nbResource.Status.NetworkResourceID != nil {
		resource, err = r.OpenZro.Networks.Resources(nbResource.Spec.NetworkID).Get(ctx, *nbResource.Status.NetworkResourceID)
		if err != nil && !strings.Contains(err.Error(), "not found") {
			logger.Error(erropenZroAPI, "error getting network resource", "err", err)
			return nil, err
		}
	}
	...
	if nbResource.Status.NetworkResourceID == nil && resource == nil {
		resource, err := r.OpenZro.Networks.Resources(nbResource.Spec.NetworkID).Create(ctx, api.NetworkResourceRequest{
			Address:     nbResource.Spec.Address,
			Enabled:     true,
			Groups:      groupIDs,
			Description: &networkDescription,
		...
		if err != nil && strings.Contains(err.Error(), "already exists") {
			log.Log.Error(erropenZroAPI, "network resource with the same name already exists", "err", err)
			nbResource.Status.Conditions = openzrov1.OZConditionFalse("DuplicateName", "Resource name already exists")
			return nil, errDuplicateResource
		}

		if err != nil {
			logger.Error(erropenZroAPI, "error creating resource", "err", err)
			return nil, err
		}
		...
		if resource.Address != nbResource.Spec.Address ||
			...
			*resource.Description != networkDescription ||
			...
			_, err = r.OpenZro.Networks.Resources(nbResource.Spec.NetworkID).Update(ctx, *nbResource.Status.NetworkResourceID, api.NetworkResourceRequest{
				Address:     nbResource.Spec.Address,
				Enabled:     true,
				Groups:      groupIDs,
				Description: &networkDescription,
...
// handleGroups create OZGroup objects for each group specified in OZResource
func (r *OZResourceReconciler) handleGroups(ctx context.Context, req ctrl.Request, nbResource *openzrov1.OZResource, logger logr.Logger) ([]string, *ctrl.Result, error) {
	nbGroupList := openzrov1.OZGroupList{}
	err := r.Client.List(ctx, &nbGroupList, &client.ListOptions{Namespace: req.Namespace})
	if err != nil {
		logger.Error(errKubernetesAPI, "error listing OZGroup", "err", err)
		return nil, nil, err
	}
	...
	for _, groupName := range nbResource.Spec.Groups {
		...
		if err != nil && !kerrors.IsNotFound(err) {
			...
			// Add OZResource as owner to OZGroup if not already done
			ownerExists := false
...
func (r *OZResourceReconciler) reconcileDelete(ctx context.Context, req ctrl.Request, nbResource *openzrov1.OZResource) error {
	if nbResource.Status.PolicyName != nil {
	...
	if nbResource.Status.NetworkResourceID != nil {
		err := r.OpenZro.Networks.Resources(nbResource.Spec.NetworkID).Delete(ctx, *nbResource.Status.NetworkResourceID)
...
func (r *OZResourceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&openzrov1.OZResource{}).
		Watches(&openzrov1.OZGroup{}, handler.EnqueueRequestForOwner(mgr.GetScheme(), mgr.GetRESTMapper(), &openzrov1.OZResource{})).
		Watches(&openzrov1.OZPolicy{}, handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []reconcile.Request {
			if v, ok := obj.GetAnnotations()["openzro.io/generated-by"]; ok {
