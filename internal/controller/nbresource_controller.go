...
func (r *NBResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if result != nil {
		nbResource.Status.Conditions = openzrov1.NBConditionFalse("internalError", fmt.Sprintf("Error occurred handling groups: %v", err))
		return *result, err
	}
	...
	if err != nil {
		nbResource.Status.Conditions = openzrov1.NBConditionFalse("internalError", fmt.Sprintf("Error occurred handling openZro Network Resource: %v", err))
		return ctrl.Result{}, err
	}
	...
	if err != nil {
		nbResource.Status.Conditions = openzrov1.NBConditionFalse("internalError", fmt.Sprintf("Error occurred handling policy changes: %v", err))
		return ctrl.Result{}, err
	}

	nbResource.Status.Conditions = openzrov1.NBConditionTrue()

	return ctrl.Result{}, nil
}

func (r *NBResourceReconciler) handlePolicyCreate(ctx context.Context, nbResource *openzrov1.NBResource, req ctrl.Request, policy string, nbPolicy *openzrov1.NBPolicy, logger logr.Logger) error {
	...
	*nbPolicy = openzrov1.NBPolicy{
	...
	if kerrors.IsAlreadyExists(err) {
		...
		nbPolicy.Spec = openzrov1.NBPolicySpec{
			Name:          name,
			Description:   "Generated by " + req.NamespacedName.String(),
			SourceGroups:  nbResource.Spec.PolicySourceGroups,
			Bidirectional: true,
...
func (r *NBResourceReconciler) handlePolicyAddUpdate(ctx context.Context, req ctrl.Request, nbResource *openzrov1.NBResource, policy string, groupIDs []string, logger logr.Logger) error {
...
func (r *NBResourceReconciler) handlePolicyDelete(ctx context.Context, req ctrl.Request, nbResource *openzrov1.NBResource, specPolicies []string, policy string, logger logr.Logger) error {
	var nbPolicy openzrov1.NBPolicy
...
func (r *NBResourceReconciler) handlePolicy(ctx context.Context, req ctrl.Request, nbResource *openzrov1.NBResource, groupIDs []string, logger logr.Logger) error {
...
func (r *NBResourceReconciler) handleGroupUpdate(ctx context.Context, nbResource *openzrov1.NBResource, groupIDs []string, resource *api.NetworkResource, logger logr.Logger) error {
...
func (r *NBResourceReconciler) handleopenZroResource(ctx context.Context, nbResource *openzrov1.NBResource, groupIDs []string, logger logr.Logger) (*api.NetworkResource, error) {
	...
	if nbResource.Status.NetworkResourceID == nil && resource == nil {
		...
		if err != nil && strings.Contains(err.Error(), "already exists") {
			...
			nbResource.Status.Conditions = openzrov1.NBConditionFalse("DuplicateName", "Resource name already exists")
...
func (r *NBResourceReconciler) handleGroups(ctx context.Context, req ctrl.Request, nbResource *openzrov1.NBResource, logger logr.Logger) ([]string, *ctrl.Result, error) {
	nbGroupList := openzrov1.NBGroupList{}
...
func (r *NBResourceReconciler) reconcileDelete(ctx context.Context, req ctrl.Request, nbResource *openzrov1.NBResource) error {
	if nbResource.Status.PolicyName != nil {
...
func (r *NBResourceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&openzrov1.NBResource{}).
		Watches(&openzrov1.NBGroup{}, handler.EnqueueRequestForOwner(mgr.GetScheme(), mgr.GetRESTMapper(), &openzrov1.NBResource{})).
		Watches(&openzrov1.NBPolicy{}, handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []reconcile.Request {
